...
for line in vals.split("\n"):
    line = line.strip()
    var_components = line.split(" ")[0].split("_")
    proper_var_components = [cm.lower().capitalize() for cm in var_components]
    var = "".join(proper_var_components)
    val = str(round(float(line.split(" ")[-1].split(",")[0]) * 100 + 100)).split(".")[0]
    print(f"{var} = {val},")